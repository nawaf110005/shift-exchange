...
import {
  getMyOffers, deleteOffer, ownerAcceptOffer, ownerRejectOffer,
  getSelectedOffers, cancelSelectionDirect,
  Offer,
...
import {
  Plus, Pencil, Trash2, ListChecks, Loader2, LogIn,
  CheckCircle, XCircle, ChevronDown, ChevronUp, Bookmark,
} from 'lucide-react'
...
type MainTab     = 'my-offers' | 'selected'
...
export default function MyOffersPage() {
  ...
  const [activeTab, setActiveTab] = useState<MainTab>('my-offers')
  ...
  // ── "المختارة" tab state ─────────────────────────────────────────────
  const [selOffers,   setSelOffers]   = useState<Offer[]>([])
  const [selLoading,  setSelLoading]  = useState(true)
  const [canceling,   setCanceling]   = useState<string | null>(null)
  const [selFilter,   setSelFilter]   = useState<StatusFilter>('all')
  const [selExpanded, setSelExpanded] = useState<Set<string>>(new Set())
  ...
  async function loadMyOffers(uid: string) {
    ...
    setMyOffers(data.filter(o => !toDelete.some(d => d.id === o.id)))
    setMyLoading(false)
  }

  async function loadSelOffers(uid: string) {
    setSelLoading(true)
  ...
  useEffect(() => {
    return onAuth((u) => {
      setUser(u)
      if (u) { loadMyOffers(u.uid); loadSelOffers(u.uid) }
      else   { setMyLoading(false); setSelLoading(false) }
    })
  ...
  function toggleMyExpanded(id: string) {
    setMyExpanded(prev => {
  ...
  async function handleCancel(offerId: string) {
    ...
    try {
      ...
      if (user) loadSelOffers(user.uid)
    } catch (err: any) {
      toast.error(err.message || 'حدث خطأ')
    } finally { setCanceling(null) }
  }

  function toggleSelExpanded(id: string) {
    setSelExpanded(prev => {
      const next = new Set(prev)
      if (next.has(id)) next.delete(id); else next.add(id)
      return next
  ...
  // ── Filtered lists ────────────────────────────────────────────────────
  const visibleMyOffers = myOffers
    .filter(o => !isPastOffer(o))
    .filter(o => {
      if (myFilter === 'confirmed')   return o.status === 'confirmed'
      if (myFilter === 'unconfirmed') return o.status !== 'confirmed'
      return true
    })

  const visibleSelOffers = selOffers
    .filter(o => !isPastOffer(o))
    .filter(o => {
      if (selFilter === 'confirmed')   return o.status === 'confirmed'
      if (selFilter === 'unconfirmed') return o.status !== 'confirmed'
      return true
    })
  ...
  function renderFilterTabs(current: StatusFilter, setter: (f: StatusFilter) => void) {
    return (
    ...
    <div className="pb-28">
      ...
      <div className="flex gap-1 mb-6 bg-gray-100 rounded-xl p-1">
        <button
          ...
          className={clsx(
            'flex-1 flex items-center justify-center gap-2 py-2.5 rounded-lg text-sm font-semibold transition-colors min-h-[40px]',
            activeTab === 'my-offers'
              ? 'bg-white text-[#1B3A6B] shadow-sm'
              : 'text-gray-500 hover:text-gray-700'
          )}
        ...
        <div>
          <div className="flex items-center justify-between mb-4">
            <div>
              ...
              <p className="text-gray-500 text-sm mt-0.5">{visibleMyOffers.length} عرض</p>
            </div>
            <button
          ...
          {renderFilterTabs(myFilter, setMyFilter)}

          {myLoading ? (
            <div className="flex justify-center py-24">
              <div className="animate-spin rounded-full h-10 w-10 border-4 border-[#2E86AB] border-t-transparent" />
            </div>
          ) : visibleMyOffers.length === 0 ? (
            <div className="text-center py-24 text-gray-400">
              <ListChecks className="w-14 h-14 mx-auto mb-4 opacity-30" />
              <p className="text-base mb-4">لا توجد عروض{myFilter !== 'all' ? ' في هذه الفئة' : ''}</p>
              {myFilter === 'all' && (
                <button
            ...
            <div className="space-y-3">
              {visibleMyOffers.map(offer => {
                ...
                const isExpanded   = myExpanded.has(offer.id!)
                  ...
                  <div key={offer.id} className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
                    ...
                    <div className="flex items-center justify-between px-4 pt-4 pb-3">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        {isConfirmed && (
                          <button
                            onClick={() => toggleMyExpanded(offer.id!)}
      ...
      {activeTab === 'selected' && (
        <div>
          <div className="mb-4">
            <h1 className="text-xl sm:text-2xl font-bold text-[#1B3A6B]">المختارة</h1>
            <p className="text-gray-500 text-sm mt-0.5">العروض التي اخترتها للتبديل · {visibleSelOffers.length}</p>
          </div>

          {renderFilterTabs(selFilter, setSelFilter)}

          {selLoading ? (
            <div className="flex justify-center py-24">
              <div className="animate-spin rounded-full h-10 w-10 border-4 border-[#2E86AB] border-t-transparent" />
            </div>
          ) : visibleSelOffers.length === 0 ? (
            <div className="text-center py-24 text-gray-400">
              <Bookmark className="w-14 h-14 mx-auto mb-4 opacity-30" />
              <p className="text-base mb-4">لا توجد عروض{selFilter !== 'all' ? ' في هذه الفئة' : ''}</p>
              {selFilter === 'all' && (
            ...
            <div className="space-y-3">
              {visibleSelOffers.map(offer => {
                const repDay      = offer.selectedReplacementDay
                const isConfirmed = offer.status === 'confirmed'
                const isExpanded  = selExpanded.has(offer.id!)
                  ...
                  <div key={offer.id} className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
                    ...
                    <div className="flex items-center justify-between px-4 pt-4 pb-3">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        {isConfirmed && (
                          <button
                            onClick={() => toggleSelExpanded(offer.id!)}
                            className="p-2 text-gray-400 active:text-[#2E86AB] rounded-xl transition-colors min-w-[36px] min-h-[36px] flex items-center justify-center"
                            title={isExpanded ? 'إخفاء التفاصيل' : 'عرض التفاصيل'}
                          >
                            {isExpanded ? <ChevronUp className="w-4 h-4" /> : <ChevronDown className="w-4 h-4" />}
